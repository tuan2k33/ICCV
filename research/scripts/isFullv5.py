...
def group_and_sort_boxes(boxes, y_threshold=BOX_Y_THRESHOLD, area_threshold=AREA_THRESHOLD):
    ...
    if class_id == 0:
        # === XỬ LÝ TOP FACE ===
        wh_ratios = [b[2][2] / b[2][3] if b[2][3] > 0 else 0 for b in boxes]
        median_ratio = np.median(wh_ratios)

        normal_faces, rare_faces = [], []
        for box, r in zip(boxes, wh_ratios):
            if (median_ratio >= 1 and r < 1) or (median_ratio < 1 and r >= 1):
                rare_faces.append(box)
            else:
                normal_faces.append(box)

        # === Gom nhóm các box theo diện tích bằng DBSCAN ===
        if normal_faces:
            area_vec = np.array([[b[1]] for b in normal_faces])
            labels = DBSCAN(eps=area_threshold, min_samples=1).fit(area_vec).labels_

            groups = {}
            for idx, lb in enumerate(labels):
                groups.setdefault(lb, []).append(normal_faces[idx])

            for group in groups.values():
                # Lọc theo y-center (giữ các box gần mean_y)
                cy_values = np.array([b[2][1] for b in group])
                mean_cy = np.mean(cy_values)
                filtered = [b for b in group if abs(b[2][1] - mean_cy) <= TOP_Y_THRESHOLD]
                if not filtered:
                    continue
                # Sắp xếp các box theo center x tăng dần
                filtered.sort(key=lambda b: b[2][0])
                top_rows.append(filtered)

            # Sort các hàng theo center y tăng dần
            top_rows.sort(key=lambda row: np.mean([b[2][1] for b in row]))

        # === Gom nhóm rare_faces theo center-y (giống front) ===
        rare_rows = []
        if rare_faces:
            y_coords = np.array([[b[2][1]] for b in rare_faces])
            labels = DBSCAN(eps=TOP_Y_THRESHOLD, min_samples=1).fit(y_coords).labels_

            grouped_rare = {}
            for idx, lb in enumerate(labels):
                grouped_rare.setdefault(lb, []).append(rare_faces[idx])

            for group in grouped_rare.values():
                group.sort(key=lambda b: b[2][0])  # sort theo center-x
                rare_rows.append(group)

            rare_rows.sort(key=lambda row: np.mean([b[2][1] for b in row]))  # sort theo center-y

        return top_rows, rare_rows

    else:
        # === XỬ LÝ FRONT FACE HOẶC FULL BOX ===
        if class_id == 1:
        ...
        for row_boxes in grouped_rows.values():
            row_boxes.sort(key=lambda b: b[2][0])  # Sort theo center x
