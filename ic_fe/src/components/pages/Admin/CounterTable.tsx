import {
  useEffect,
  useImperativeHandle,
  useMemo,
  useRef,
  useState,
  type ChangeEvent,
  type RefObject,
} from 'react'
import { useFormik } from 'formik'
import { twMerge } from 'tailwind-merge'
import { ImSpinner8 } from 'react-icons/im'

import { useModal } from '~/hooks/useModal'
import { useMutation } from '~/hooks/useMutation'
import { useIsInView } from '~/hooks/useIsInView'
import { useGenerateUsername } from '~/hooks/useGenerateUsername'
import { toastSuccess } from '~/utils/toastSuccess'
import { eventBus, EventBusType } from '~/utils/eventBus'
import { formatString } from '~/utils/formatString'
import { toastError } from '~/utils/showErrorToast'
import { generateDisplayFullname } from '~/utils/generateDisplayFullname'
import { UpdateUserSchema } from '~/configs/schemas'
import { endpoints } from '~/configs/endpoints'
import type { UserCommon } from '~/types/common'

import CheckIcon from '~/assets/icons/check.svg'
import PencilIcon from '~/assets/icons/pencil.svg'
import XCloseIcon from '~/assets/icons/x-close.svg'
import Badge from '~/components/common/Badge'
import BaseModal from '~/components/common/BaseModal'
import Input from '~/components/common/Input'
import Table from '~/components/common/Table'
import Tooltip from '~/components/common/Tooltip'
import Loading from '~/components/common/Loading'
import TruncatedDisplay from '~/components/common/TruncatedDisplay'

export interface CounterTableRef {
  getSelectedKeys: () => (string | number)[]
}

interface Props {
  company: string
  users: UserCommon[]
  total: number
  search: string
  loading: boolean
  ref?: RefObject<CounterTableRef | null>
  onUpdatedUser: () => void
  fetchNextPage?: () => void
}

export default function CounterTable({
  company,
  users,
  total,
  search,
  loading,
  ref,
  onUpdatedUser,
  fetchNextPage,
}: Readonly<Props>) {
  const [selectedKeys, setSelectedKeys] = useState<(string | number)[]>([])
  const [editingRow, setEditingRow] = useState<null | string | number>(null)
  const initEditUsernameRef = useRef<string>(null)
  const [openTooltip, setOpenTooltip] = useState<null | 'fullname' | 'phoneNumber'>(null)
  const confirmEditModal = useModal()
  const loadMoreRef = useRef<HTMLDivElement | null>(null)

  const form = useFormik({
    initialValues: {
      /**
       * `id` is only used for validation purpose
       */
      id: -1,
      fullname: '',
      phoneNumber: '',
      username: '',
    },
    validationSchema: UpdateUserSchema,
    onSubmit() {
      confirmEditModal.openModal()
    },
  })

  useGenerateUsername(
    form.values.fullname,
    form.values.phoneNumber,
    company.toLowerCase() as 'linfox' | 'unilever',
    {
      disabled: !!form.errors.fullname,
      userId: form.values.id,
      onGenerate: (username) => {
        form.setFieldValue('username', username)
      },
    },
  )

  const dataSource = useMemo(() => {
    return users.map((record) => ({
      key: record.id,
      ...record,
      displayFullName: generateDisplayFullname(record.fullname, search),
    }))
  }, [search, users])

  useEffect(() => {
    const handleDeleteAllUserRequest = () => {
      setSelectedKeys(dataSource.map((record) => record.key))

      eventBus.emit(EventBusType.SELECTED_COUNTER, {
        company: company.toLowerCase(),
        count: dataSource.length,
        requestDelete: true,
      })
    }
    eventBus.on(EventBusType.DELETE_ALL_USER_REQUEST, handleDeleteAllUserRequest)

    return () => {
      eventBus.off(EventBusType.DELETE_ALL_USER_REQUEST, handleDeleteAllUserRequest)
    }
  }, [dataSource, company])

  useEffect(() => {
    const handleClearSelectedUser = () => {
      setSelectedKeys([])
    }
    eventBus.on(EventBusType.CLEAR_SELECTED_USER, handleClearSelectedUser)
    return () => {
      eventBus.off(EventBusType.CLEAR_SELECTED_USER, handleClearSelectedUser)
    }
  }, [])

  useEffect(() => {
    const handleCancelDeleteUser = () => {
      setSelectedKeys([])
    }
    eventBus.on(EventBusType.CANCEL_DELETE_USER, handleCancelDeleteUser)

    return () => {
      eventBus.off(EventBusType.CANCEL_DELETE_USER, handleCancelDeleteUser)
    }
  }, [])

  useImperativeHandle(
    ref,
    () => ({
      getSelectedKeys: () => selectedKeys,
    }),
    [selectedKeys],
  )

  const handleLoadMore = () => {
    if (!loading) fetchNextPage?.()
  }

  useIsInView(loadMoreRef, {
    deps: [loading],
    listener(isInView) {
      if (isInView && !loading) {
        handleLoadMore()
      }
    },
  })

  const updateUserMutation = useMutation({
    method: 'put',
    url: formatString(endpoints.USER_UPDATE, { user_id: editingRow as string }),
  })

  const handleSelectedKeysChange = (selectedKeys: (string | number)[]) => {
    setSelectedKeys(selectedKeys)
    eventBus.emit(EventBusType.SELECTED_COUNTER, {
      company: company.toLowerCase(),
      count: selectedKeys.length,
    })
  }

  const handleEditRow = (row: (typeof dataSource)[number]) => {
    form.resetForm()
    setEditingRow(row.key)
    initEditUsernameRef.current = row.username
    form.setValues(
      {
        ...form.values,
        id: row.id,
        fullname: row.fullname,
        phoneNumber: row.phone_number,
        username: row.username,
      },
      true,
    )
  }

  const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.name === 'phoneNumber') {
      form.setValues({ ...form.values, phoneNumber: e.target.value })
    } else if (e.target.name === 'fullname') {
      const fullname = e.target.value
      form.setValues({ ...form.values, fullname: fullname })
    }
  }

  const handleCancelEdit = () => {
    setEditingRow(null)
    form.resetForm()
  }

  const handleUpdate = () => {
    updateUserMutation.mutate(
      {
        body: {
          fullname: form.values.fullname,
          phone_number: form.values.phoneNumber,
          username: form.values.username,
        },
      },
      {
        onError: () => {
          toastError('Sửa thông tin thất bại, vui lòng thử lại')
        },
        onSuccess() {
          form.resetForm()
          confirmEditModal.closeModal()
          setEditingRow(null)
          toastSuccess('Sửa thông tin thành công')
          onUpdatedUser()
        },
      },
    )
  }

  return (
    <>
      <div className="border-r border-border-secondary flex flex-col overflow-hidden">
        <div className="flex items-center justify-center gap-3 h-11 border-b border-border-secondary shrink-0">
          <p className="text-lg font-medium leading-[18px]">{company}</p>
          <Badge className="text-error-secondary border-[#FF6A60] bg-[#FFB5B0]">{total}</Badge>
        </div>
        <div className="relative overflow-y-auto grow">
          <Table
            classNames={{
              thead: 'sticky top-0 left-0',
            }}
            rowSelection={{ selectedKeys: selectedKeys, onChange: handleSelectedKeysChange }}
            columns={[
              {
                key: 'full_name',
                label: 'Họ và tên',
                dataKey: 'full_name',
                align: 'left',
                width: '36%',
                cellClassName: 'has-[input]:py-0',
                render(record) {
                  if (editingRow === record.key)
                    return (
                      <Tooltip content={form.errors.fullname} open={openTooltip === 'fullname'}>
                        <Input
                          name="fullname"
                          value={form.values.fullname}
                          classnames={{
                            wrapper: 'h-8',
                          }}
                          autoFocus
                          maxLength={100}
                          touched
                          errorStatusWithOutMessage={!!form.errors.fullname}
                          onFocus={() => setOpenTooltip('fullname')}
                          onBlur={() => setOpenTooltip(null)}
                          onChange={handleInputChange}
                        />
                      </Tooltip>
                    )
                  return (
                    <div
                      dangerouslySetInnerHTML={{
                        __html: record.displayFullName ?? record.fullname,
                      }}
                    />
                  )
                },
              },
              {
                key: 'phone_number',
                dataKey: 'phone_number',
                label: 'Số điện thoại',
                align: 'left',
                width: '23%',
                cellClassName: 'has-[input]:py-0',
                render(record) {
                  if (record.key === editingRow)
                    return (
                      <Tooltip
                        content={form.errors.phoneNumber}
                        open={openTooltip === 'phoneNumber'}
                      >
                        <Input
                          name="phoneNumber"
                          value={form.values.phoneNumber}
                          classnames={{
                            wrapper: 'h-8',
                          }}
                          maxLength={10}
                          touched
                          errorStatusWithOutMessage={!!form.errors.phoneNumber}
                          onFocus={() => setOpenTooltip('phoneNumber')}
                          onBlur={() => setOpenTooltip(null)}
                          onChange={handleInputChange}
                        />
                      </Tooltip>
                    )
                  return (
                    <TruncatedDisplay
                      content={record.phone_number}
                      className="line-clamp-1 break-all"
                    />
                  )
                },
              },
              {
                key: 'username',
                dataKey: 'username',
                label: 'Username',
                align: 'left',
                cellClassName: 'text-tertiary',
                width: '23%',
                render(record) {
                  if (record.key === editingRow) return form.values.username
                  return record.username
                },
              },
              {
                key: 'action',
                width: 68,
                cellClassName: 'px-0',
                render: (record) => {
                  if (record.key === editingRow)
                    return (
                      <div className="flex items-center justify-center gap-2.5 text-xl">
                        <button onClick={form.submitForm} disabled={!form.isValid}>
                          <CheckIcon className="text-success" />
                        </button>
                        <button>
                          <XCloseIcon className="text-error" onClick={handleCancelEdit} />
                        </button>
                      </div>
                    )
                  return (
                    <button
                      className="block text-text-secondary text-xl"
                      onClick={() => handleEditRow(record)}
                    >
                      <PencilIcon />
                    </button>
                  )
                },
              },
            ]}
            dataSource={dataSource}
          />
          <div className={twMerge('h-20', dataSource.length === total && 'hidden')}>
            <div
              className="h-full flex items-center justify-center pointer-events-none select-none"
              ref={loadMoreRef}
            >
              <ImSpinner8 className="animate-spin" />
            </div>
          </div>
          {loading && !dataSource.length && <Loading className="absolute inset-0" />}
        </div>
      </div>
      <BaseModal
        open={confirmEditModal.open}
        title="SỬA THÔNG TIN"
        classNames={{
          body: 'w-[430px]',
        }}
        confirmButton={{
          label: 'Lưu',
          onClick: handleUpdate,
          loading: updateUserMutation.pending,
        }}
        onRequestClose={!updateUserMutation.pending ? confirmEditModal.closeModal : undefined}
      >
        <p>
          Chỉnh sửa Số điện thoại có thể thay đổi Mật khẩu. Thay đổi sẽ áp dụng từ lần đăng nhập
          tiếp theo. <br />
          <br /> Bạn có chắc chắn muốn sửa?
        </p>
      </BaseModal>
    </>
  )
}
